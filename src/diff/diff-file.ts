...
export class DiffFile {
  ...
  constructor(
    ...
    if (!oldContent && !newContent) {
      console.error(
        'should pass "oldFileContent" or "newFileContent" to make the diff work!'
      );
    }
  }
    ...
    if (this.#oldFileResult) {
      ...
      while (oldLineNumber < this.#oldFileResult.maxLineNumber) {
        ...
        if (newDiffLine) {
          ...
          oldLineNumber = newDiffLine.oldLineNumber
            ? newDiffLine.oldLineNumber + 1
            : oldLineNumber;
        } else {
    ...
    if (this.#newFileResult) {
      ...
      while (newLineNumber < this.#newFileResult.maxLineNumber) {
        ...
        if (oldDiffLine) {
          ...
          newLineNumber = oldDiffLine.newLineNumber
            ? oldDiffLine.newLineNumber + 1
            : newLineNumber;
        } else {
  ...
  #composeDiff() {
          ...
          if (line.type === DiffLineType.Add) {
            ...
            if (additions.length === deletions.length) {
              ...
              for (let i = 0; i < len; i++) {
                ...
                const { stringARange, stringBRange } = relativeChanges(
                  addition.text,
                  deletion.text
                );
    ...
    this.#diffLines = this.#diffListResults
      .reduce<DiffLine[]>(
        (p, c) =>
          p.concat(
            ...c.hunks.reduce<DiffLine[]>(
              (_p, _c) => _p.concat(..._c.lines),
              []
            )
          ),
        []
  ...
  buildSplitDiffLines = () => {
    this.lineLength = Math.max(
      this.lineLength,
      this.#oldFileResult?.rawLength || 0,
      this.#newFileResult?.rawLength || 0
    );
    ...
    numIterator(this.lineLength, () => {
      ...
      if (
        (oldLineHasChange && newLineHasChange) ||
        (!oldLineHasChange && !newLineHasChange)
      ) {
      ...
      if (
        ...
        const previous = newDiffLine.index
          ? this.#diffLines?.[current.index - 1]
          : undefined;
  ...
  buildUnifiedDiffLines = () => {
    ...
    while (
      oldFileLineNumber < maxOldFileLineNumber ||
      newFileLineNumber < maxNewFileLineNumber
    ) {
      const oldRawLine = this.#getOldRawLine(oldFileLineNumber);
      const oldDiffLine = this.#getOldDiffLine(oldFileLineNumber);
      const newRawLine = this.#getNewRawLine(newFileLineNumber);
  ...
  onSplitHunkExpand = (dir: "up" | "down" | "all", index: number) => {
    ...
    if (dir === "all") {
      for (
        let i = current.splitInfo.startHiddenIndex;
        i < current.splitInfo.endHiddenIndex;
        i++
      ) {
      ...
      for (
        let i = current.splitInfo.startHiddenIndex;
        i < current.splitInfo.startHiddenIndex + composeLen;
        i++
      ) {
        const leftLine = this.#splitLeftLines[i];
        const rightLine = this.#splitRightLines[i];
      ...
      for (
        let i = current.splitInfo.endHiddenIndex - composeLen;
        i < current.splitInfo.endHiddenIndex;
        i++
      ) {
        const leftLine = this.#splitLeftLines[i];
  ...
  onSplitLastExpand = () => {
    ...
    for (
      let i = this.splitLastStartIndex;
      i < this.splitLastStartIndex + composeLen;
      i++
    ) {
      const leftLine = this.#splitLeftLines[i];
      const rightLine = this.#splitRightLines[i];
    ...
    this.splitLastStartIndex =
      this.splitLastStartIndex >= this.lineLength
        ? Infinity
        : this.splitLastStartIndex;
  ...
  onUnifiedHunkExpand = (dir: "up" | "down" | "all", index: number) => {
    ...
    if (dir === "all") {
      for (
        let i = current.unifiedInfo.startHiddenIndex;
        i < current.unifiedInfo.endHiddenIndex;
        i++
      ) {
      ...
      for (
        let i = current.unifiedInfo.startHiddenIndex;
        i < current.unifiedInfo.startHiddenIndex + composeLen;
        i++
      ) {
        const unifiedLine = this.#unifiedLines[i];
      ...
      for (
        let i = current.unifiedInfo.endHiddenIndex - composeLen;
        i < current.unifiedInfo.endHiddenIndex;
        i++
      ) {
        const unifiedLine = this.#unifiedLines[i];
  ...
  onUnifiedLastExpand = () => {
    ...
    for (
      let i = this.unifiedLastStartIndex;
      i < this.unifiedLastStartIndex + composeLen;
      i++
    ) {
      const unifiedLine = this.#unifiedLines[i];
      if (unifiedLine?.isHidden) unifiedLine.isHidden = false;
    ...
    this.unifiedLastStartIndex =
      this.unifiedLastStartIndex >= this.unifiedLineLength
        ? Infinity
        : this.unifiedLastStartIndex;
