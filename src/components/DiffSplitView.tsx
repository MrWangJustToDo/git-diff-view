...
const DiffSplitViewTable = ({
  ...
  const className =
    side === SplitSide.new ? "new-diff-table" : "old-diff-table";

  const showExpandLast =
    diffFile.splitLastStartIndex &&
    Number.isFinite(diffFile.splitLastStartIndex);

  return (
    <table
      className={className + " border-collapse w-full"}
      data-mode={SplitSide[side]}
    >
      <colgroup>
