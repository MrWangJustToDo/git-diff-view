...
export const DiffUnifiedExtendLine = defineComponent(
  (props: { index: number; diffFile: DiffFile; lineNumber: number }) => {
    ...
    const oldExtend = computed(() => extendData?.value?.oldFile?.[unifiedItem.value.oldLineNumber ?? -1]);

    const newExtend = computed(() => extendData?.value?.newFile?.[unifiedItem.value.newLineNumber ?? -1]);
